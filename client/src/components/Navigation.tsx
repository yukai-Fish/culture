...
const navItems = [
  ...
  { path: "/interactive", label: "感应场", icon: "礼" },
  { path: "/library", label: "藏书阁", icon: "书" },
  { path: "/meditation", label: "禅修房", icon: "静" },
];
      ...
      <div className="glass-card border-b border-white/20">
        <div className="container flex items-center justify-between h-16">
          <Link href="/" className="flex items-center gap-2 group">
            ...
            {theme && <span className="text-sm opacity-60 font-serif">{theme.name}</span>}
          </Link>
          ...
          <button className="md:hidden p-2 text-[var(--color-ink-medium)]" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
                  ...
                  <Link key={item.path} href={item.path}>
                    <div
                      className={`px-4 py-3 rounded-sm font-serif text-sm flex items-center gap-3 ${
                        isActive ? "bg-[var(--color-mountain-near)]/10 text-[var(--color-mountain-near)]" : "text-[var(--color-ink-medium)]"
                      }`}
